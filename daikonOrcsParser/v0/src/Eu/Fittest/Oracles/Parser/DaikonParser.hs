...
pUnmodifiedVars = do {
       ...
       return []
       -- [ O_unModified . VariableName . breakName $ name | name <- filterVar vars]
    }
    where
...
pConstSet =  LitIntSet <$> pConstSet__ pInt
              <|> LitIntListSet <$> pConstSet__ (pConstList__ pInt)
              <|> LitDoubleSet  <$> pConstSet__ pDouble
              <|> LitBoolSet    <$> pConstSet__ pBool
              <|> LitStringSet  <$> pConstSet__ pString              
...
markOrcsSuiteWithViolations violations orcsSuite = map markGroup orcsSuite
   ...
   markO vios (o,k,verdict) = worker vios
      where
      worker [] = (o,k,verdict)
      worker ((o',info):rest) = if matchO o o' then (o,k, Just (Violated,info)) 
                                               else  worker rest
                                               
                                               
   -- function to check if o' (violated oracle, comes from violation report) matches 
   -- a reported oracle o. Most of the time this is the case if o'==o; but Daikon
   -- invariant check contains bugs; one of the bugs is violation of oracles like "a elements > 0";
   -- which we will try to fix below:
   matchO (Pre  (O_AllElems rel1 t1 u1)) (Pre  (O_AllElems rel2 t2 u2)) = (rel1 == rel2) && (t1 == t2) -- ignoring u1 and u2
   matchO (Post (O_AllElems rel1 t1 u1)) (Post (O_AllElems rel2 t2 u2)) = (rel1 == rel2) && (t1 == t2) -- ignoring u1 and u2
   -- for all other cases:
   matchO o o' = (o == o')
...
parseViolationsFromFiles files = do {
     ...
     -- trace (">>>violations" ++ show violations) $
...
pConstantValue = pConstNumber <|> pConstBool <|> pConstString <|> pConstList 
...
pViolationLine = do {
   ...
   --
   fixOneOfViolation (e,q, O_oneOf x set, v, value, info) 
     | isLitSinglSet set = (e, q, O_Relation EQ_  x (singSet2Elem set), v, value, info) 
     | otherwise         = (e,q, O_oneOf x (fix set), v, value, info)
      where
      violatingvalue_ = show value
      -- fixing the oneOf set
      fix (LitIntSet s)     = LitIntSet [v | v<-s , show v /= violatingvalue_]
      fix (LitIntListSet s) = LitIntListSet [v | v<-s , show v /= violatingvalue_]
      ...
      isLitSinglSet :: Term -> Bool
      isLitSinglSet (LitIntSet s)     | length s == 2 = True
      isLitSinglSet (LitIntListSet s) | length s == 2 = True
      isLitSinglSet (LitBoolSet s)    | length s == 2 = True
      isLitSinglSet (LitStringSet s)  | length s == 2 = True
      isLitSinglSet (LitDoubleSet s)  | length s == 2 = True
      isLitSinglSet _                                = False 

      singSet2Elem (LitIntSet s)     = LitInt $ head s
      singSet2Elem (LitIntListSet s) = LitIntList $ head s
      singSet2Elem (LitBoolSet s)    = LitBool $ head s  
      singSet2Elem (LitStringSet s)  = LitString $ head s
      singSet2Elem (LitDoubleSet s)  = LitDouble $ head s 
...
onlyReferToOldVars o = check o
  ...
  check (O_AllElems _ t1 t2) = checkt t1 && checkt t2
