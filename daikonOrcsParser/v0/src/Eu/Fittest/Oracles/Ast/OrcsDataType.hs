...
-- the type should be polymorphic, i.e. data Term a = 
data Term =
       ...
       | LitIntSet    [Int]
       | LitIntListSet [[Int]] -- this constructor should disappear when the Term become polymorphic
       | LitBoolSet   [Bool]
       | LitStringSet [String]
       | LitDoubleSet [Double]
       | SizeOf        Term        -- e.g. size(a), expressing the size of an array
...
data Oracle =
      ...
      | O_AllElems Relation_ Term Term  -- e.g. a elements > 0, to mean all elements of the array a are > 0
      | O_oneOf Term Term          -- e.g. x one of {1,99}
      | O_isConstant Term          -- to say that x never changes
      | O_unModified Term          -- to say that t = old(t) NOTE: this is a specific property that doesn't match the one given as example
